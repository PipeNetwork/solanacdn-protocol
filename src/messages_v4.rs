...
/// Version 4 of the SolanaCDN wire protocol.
///
/// This module exists to support dual-stack decoding/encoding during rollouts where POPs and
/// validators may not be upgraded simultaneously.
...
pub struct FairBatchAttestationPayload {
    ...
    pub batch_id: u128,
    /// POP-assigned monotonic sequence number of `txs[0]` for this origin POP.
...
impl FairBatchCommit {
    pub fn sign(
        payload: FairBatchCommitPayload,
        signing_key: &SigningKey,
    ) -> Result<Self, CryptoError> {
        let bytes = postcard::to_stdvec(&payload)?;
        let signature = signing_key.sign(&bytes);
